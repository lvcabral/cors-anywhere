...
- 🔧 Fixed rate limiter `setInterval` leak (added destroy method for cleanup)
- 🧹 Added help file cache size limit (prevents unbounded growth)
- 🗑️ Cleanup request state in error handler (prevents state accumulation)
...
- `httpProxyOptions` - Under the hood, [http-proxy](https://github.com/nodejitsu/node-http-proxy)
  ...
  to http-proxy. The documentation for these options can be found in [http-proxy documentation](https://github.com/nodejitsu/node-http-proxy#options).
